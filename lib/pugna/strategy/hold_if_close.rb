    ...
    class HoldIfClose
      ...
      def next_move(board, active_piece)
        ...
        friendlies = board.friendlies(active_piece)
                          .select { |p| p.neighbour? active_piece }

        return Pugna::Move.direction(active_piece, board.closest_friend(active_piece)) if friendlies.size < 2
        return :STAY if enemy_dist.between? 2, 4
